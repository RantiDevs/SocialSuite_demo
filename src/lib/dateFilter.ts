...
export function filterReelsByDate<T extends { datePosted: string }>(
  ...
  return reels.filter(reel => {
    ...
    let reelDate: Date;
    if (/^\d+$/.test(reel.datePosted)) {
      // Handle potential Unix timestamp strings just in case
      reelDate = new Date(parseInt(reel.datePosted) * 1000);
    } else {
      reelDate = parseISO(reel.datePosted);
    }
